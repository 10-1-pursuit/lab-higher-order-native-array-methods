...
function findPinkElephantsByTimestreet(songs) {
  return (songs.find(pE => pE.title === "Pink Elephants"))
}
...
function findFirstSongUnderThreeMinutes(songs) {
  return (songs.find(underThreeMin => underThreeMin.runtimeInSeconds < 180))
}
...
function findFirstTitleTrack(songs) {
  return (songs.find(titleMatchesAlbum => titleMatchesAlbum.title === titleMatchesAlbum.album))
}
