...
function findPinkElephantsByTimestreet(songs) {
  const twoPinkElephant = songs.find((pinkElephant) =>
  (pinkElephant.title === "Pink Elephants")
  );
  return twoPinkElephant;
}
...
function findFirstSongUnderThreeMinutes(songs) {
  const firstSongUnderThree = songs.find((firstSong) =>
  (firstSong.runtimeInSeconds < 180)
  );
  return firstSongUnderThree;
}
...
function findFirstTitleTrack(songs){
  const titleTrack = songs.find((firstSong) =>
  (firstSong.title === firstSong.album)
  );
  return titleTrack;
}
