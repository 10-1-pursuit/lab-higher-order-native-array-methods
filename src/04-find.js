...
function findPinkElephantsByTimestreet(songs) {
  const pinkElephants = songs.find(song => song.title === "Pink Elephants" && song.artist === "Timestreet");
...
function findFirstSongUnderThreeMinutes(songs) {
  const firstSong = songs.find(song => song.runtimeInSeconds < 180 && song.artist);
...
function findFirstTitleTrack(songs) {
  const firstTitleTrack = songs.find(song => song.title === song.album);
  return firstTitleTrack || null;
}
