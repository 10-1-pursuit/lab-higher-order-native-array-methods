...
function sortByRuntimeAscending(songs) {
  ...
  const sortByTimeGoingUp = songs.sort((a, b) => a.runtimeInSeconds - b.runtimeInSeconds
  );
  return sortByTimeGoingUp
};
...
function sortByArtistNameDescending(songs) {
  const sortArtistByLastLetter = songs.sort((a, b) => {
    if (a.artist.toLowerCase() > b.artist.toLowerCase()) return -1;
    return 1;
  });
  return sortArtistByLastLetter
};
...
function sortBySongTitleAscending(songs) {
  const songsThatStartWithA = songs.sort((a, b) => {
