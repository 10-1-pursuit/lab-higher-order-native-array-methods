...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every(overTwo => songs.includes(overTwo.runtimeInSeconds > 120))
}
...
function anySongIsOverFourMinutes(songs) {
  return songs.some(overTwo => songs.includes(overTwo.runtimeInSeconds > 240))
}
...
function anySongIsByPeanut(songs) {
  return songs.some(artists => artists.artist === "Peanut")
}
