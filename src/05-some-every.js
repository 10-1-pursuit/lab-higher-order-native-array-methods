...
function allSongsAreOverTwoMinutes(songs) {
  return songs.every(overTwoMin => overTwoMin.runtimeInSeconds > 120 )
...
function anySongIsOverFourMinutes(songs) {
  return songs.some(overFourMins => overFourMins.runtimeInSeconds > 240)
}
...
function anySongIsByPeanut(songs) {
  return ( songs.some(anySongByP => anySongByP.artist === "Peanut"))
}
