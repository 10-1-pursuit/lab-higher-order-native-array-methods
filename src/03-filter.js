 ...
 */
function getSongsBySaib(songs) { return songs.filter(song => song.artist === "Saib") 
};
/**
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter(overThreeMin => overThreeMin.runtimeInSeconds >180 )
}
...
function getTitleTracks(songs) {
  return songs.filter( sameEntry => sameEntry.title === sameEntry.album)
}
