...
function getSongsBySaib(songs) {
  const saibSongs = songs.filter(el => el.artist === 'Saib');
    return saibSongs; 
}
...
function getSongsOverThreeMinutes(songs) {
  const songTime = songs.filter(el => el.runtimeInSeconds > 180);
    return songTime;
}
...
function getTitleTracks(songs) {
  const titleTracks = songs.filter(el => el.title === el.album);
    return titleTracks;
}
