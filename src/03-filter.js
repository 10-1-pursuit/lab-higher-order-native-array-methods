...
function getSongsBySaib(songs) {
 const saibHits = songs.filter((hitsBySaib) =>
 (hitsBySaib.artist === "Saib")
 );
 return saibHits;
}
...
const songThree = songs.filter((runSongsOver) => 
(runSongsOver.runtimeInSeconds > 180)
); 
return songThree;
}
...
function getTitleTracks(songs) {
  const songTitle = songs.filter((songRecord) => 
  (songRecord.title === songRecord.album)
  ); 
  return songTitle;
}
