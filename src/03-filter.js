...
function getSongsBySaib(songs) {
  const results = songs.filter(songBy => songBy.artist.includes("Saib"))
    return results 
}
...
function getSongsOverThreeMinutes(songs) {
  const overThree = songs.filter(three => three.runtimeInSeconds > 180) 
   return overThree
}
...
function getTitleTracks(songs) {
  return songs.filter(track => track.title === track.album )
}
