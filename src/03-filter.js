...
function getSongsBySaib(songs) {
  const results = songs.filter(song=>song.artist==="Saib")
  return results
}
/**
...
function getSongsOverThreeMinutes(songs) {
  const results = songs.filter(song => song.runtimeInSeconds>= 180);
  return results
}
...
function getTitleTracks(songs) {
  const results = songs.filter(song => song.title == song.album);
  return results
}
