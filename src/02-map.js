...
function getSongTitles(songs) {
  const songTitleArray = songs.map(
    (songTitles)=>{
      return songTitles.title
    }
  )
  return songTitleArray;
}
...
function getSongDetails(songs) {
  const songTitleAndArtistArray = songs.map(
    (titlesAndArtists)=>{
      return `${titlesAndArtists.title} by ${titlesAndArtists.artist}`
    }
  )
  return songTitleAndArtistArray;
}
