 ...
 */
function getSongTitles(songs) {return songs.map(EachsongTitle => Eachsong.title) }
 ...
 */
function getSongDetails(songs) {return songs.map(song => `${song.title} by ${song.artist}`)}
 ...
 */
function getTitleAndArtist(songs) {  return songs.map(song => `${song.title} : ${song.artist}`)}
