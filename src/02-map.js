 ...
 */
function getSongTitles(songs) { return songs.map(eachSongTitle => eachSongTitle.title) };
 ...
 */
function getSongDetails(songs) { return songs.map(eachSongDetails => `${eachSongDetails.title} by ${eachSongDetails.artist}`) };
...
function getTitleAndArtist(songs) {
  const mapFunc = (eachSongDetails) => {

    return {
      [eachSongDetails.title]:  eachSongDetails.artist 
    }
  }
  return songs.map(mapFunc)
  }
