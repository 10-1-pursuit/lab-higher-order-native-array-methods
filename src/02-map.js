...
function getSongTitles(songs) {
  ...
  const result = songs.map(songName => songName.title);
  return result;
...
function getSongDetails(songs) {
  ...
  const result = songs.map(songDetails => `${songDetails.title} by ${songDetails.artist}`);
...
function getTitleAndArtist(songs) {
  ...
  const result = songs.map((titleAndArtist) => {
