...
function getSongTitles(songs) {
  let songTitles = songs.map(titles => titles.title);
  return songTitles;
}
...
function getSongDetails(songs) {
  ...
  let bestofBoth = songs.map(getBoth => `${getBoth.title} by ${getBoth.artist}` );
  return bestofBoth;
}
...
function getTitleAndArtist(songs) {
  ...
  let titleArtist = songs.map(bothOfBoth => ({ [bothOfBoth.title]: bothOfBoth.artist}))
  return titleArtist ;
