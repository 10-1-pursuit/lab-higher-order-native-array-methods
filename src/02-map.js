...
function getSongTitles(songs) {
  const songTitle = songs.map((eleTitles) =>  eleTitles.title )
   return songTitle
}
...
function getSongDetails(songs) {
  const songDets = songs.map((songDets)=> `${songDets.title} by ${songDets.artist}`)
   return songDets
}
...
function getTitleAndArtist(songs) {
  const reStructure = songs.map(newObj =>({[newObj.title]: newObj.artist}))
   return reStructure 
}
