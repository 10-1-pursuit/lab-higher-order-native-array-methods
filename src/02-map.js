...
function getSongTitles(songs) {
  const songNames = songs.map((eachSong) => eachSong.title
  );
  return songNames;
};
...
function getSongDetails(songs) {
  ...
  const songTitleAndArtist = songs.map((songTitleByArtist) => `${songTitleByArtist.title} by ${songTitleByArtist.artist}`
  )
...
function getTitleAndArtist(songs) {
  ...
  const songTitleByArtist = songs.map((songTitleByArtist) => ({
    [songTitleByArtist.title]: songTitleByArtist.artist
  }))
  return songTitleByArtist;
};
